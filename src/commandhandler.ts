...
export const handleCmd = async (message: Message, prefix: string) => {
	...
	if (!(await query<UserRow[]>(`SELECT * FROM users WHERE userId = ${message.author.id}`)).length){
		let methods = new Methods();
		await methods.createNewUser(message.author.id); // Create new account in database for user BEFORE executing a command.
	}

	const row = (await query<UserRow[]>(`SELECT * FROM users WHERE userId = ${message.author.id}`))[0];
	...
	else if (command.modOnly && !message.member!.hasPermission('MANAGE_GUILD'))
		return message.reply('You need the `MANAGE_SERVER` permission to run that command.');
	else if (command.adminOnly && !message.client.sets.adminUsers.has(message.author.id))
		return message.reply('You must be an admin of the bot to run that command.');
