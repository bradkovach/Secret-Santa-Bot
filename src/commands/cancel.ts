...
export default  {
	...
	async execute(message: Message, args: string[], prefix: string): Promise<Message | undefined> {
		...
		await query<never>(`DELETE FROM exchange WHERE exchangeId = ${exchangeRow.exchangeId}`);
		await query<never>(
